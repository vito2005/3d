...
const colors = {
  ...
  fill1Color: new THREE.Color("#FEFFFF"), // Специальный цвет для Fill1 (почти белый, но отличимый)
  fillStages,
}
...
export default function AboutCoin3D({
  ...
  useEffect(() => {
    ...
    const fillPhaseDuration = 5.8; // Увеличено для замедления анимации
    const phaseOverlap = 0.93; // Новая фаза стартует, когда предыдущая прошла phaseOverlap доли
    const pingPongRatio = pingPongStart;
    const pingPongMax = pingPongEnd;
    ...
    const pixelateShader = {
      ...
      fragmentShader: `
        ...
        // Проверка, находится ли текущая позиция в области ховера
        bool isInHoverZone(vec2 fragCoord, float cellSize) {
        ...
        vec4 applyHover(
          ...
          for (int i = 0; i < trailMax; i++) {
            ...
            if (inExtendedTrailZone) {
              ...
              vec4 fadedChoice = mix(baseColor, baseResult, totalFadeFactor);
              return fadedChoice;
            }
          }
        ...
        vec4 applyIcons(vec4 baseColor, vec4 srcColor, vec2 fc, vec2 cellIdx, float cell) {
          // Проверяем #FFA280 и #FF4600 в sRGB (с допуском)
          vec3 srgb = pow(clamp(srcColor.rgb, 0.0, 1.0), vec3(1.0 / 2.2));
          ...
          if (showX || showH) {
            ...
            if (showX) {
              ...
              vec3 iconCol = mix(vec3(1.0), xCol.rgb, xCol.a);
              return vec4(iconCol, 1.0);
            } else {
        ...
        void main() {
          ...
          // Высоты башен по фазам (можно настраивать отдельно)
          float towerHeightsPhase0[12] = float[12](3.0, 4.0, 3.5, 4.0, 3.0, 4.0, 6.0, 6.0, 5.0, 4.0, 3.5, 3.0);
          float towerHeightsPhase1[12] = float[12](4.0, 4.2, 3.0, 4.0, 4.7, 4.2, 4.4, 4.0, 3.8, 4.0, 3.5, 3.0);
          float towerHeightsPhase2[12] = float[12](4.4, 3.5, 3.2, 4.0, 6.0, 5.0, 3.0, 5.0, 4.8, 4.0, 3.5, 3.0);
          float towerHeightsPhase3[12] = float[12](2.0, 3.2, 3.3, 2.0, 2.0, 3.0, 4.2, 3.8, 2.0, 4.0, 3.5, 3.0);
          float towerHeightsPhase4[12] = float[12](3.0, 2.0, 3.0, 3.5, 2.0, 3.0, 2.0, 2.0, 2.0, 2.0, 3.5, 3.0);
          float towerHeightsPhase5[12] = float[12](3.0, 2.0, 3.0, 3.5, 2.0, 3.0, 2.0, 2.0, 2.0, 2.0, 3.5, 3.0);
          ...
          for (int i = 0; i < 6; i++) {
            ...
            // Выбор высоты башни для текущей фазы
            float towerHeight = 0.0;
            if (i == 0) {
              towerHeight = towerHeightsPhase0[int(columnIndex)];
            } else if (i == 1) {
              towerHeight = towerHeightsPhase1[int(columnIndex)];
            } else if (i == 2) {
              towerHeight = towerHeightsPhase2[int(columnIndex)];
            } else if (i == 3) {
              towerHeight = towerHeightsPhase3[int(columnIndex)];
            } else if (i == 4) {
              towerHeight = towerHeightsPhase4[int(columnIndex)];
            } else if (i == 5) {
              towerHeight = towerHeightsPhase5[int(columnIndex)];
            }
            ...
            if (i == 0) {
              vec3 fillCurrentColor = fillColors[i];
              stagedColor = computeCellFillColor(cellUv, stagedColor, fillCurrentColor);
              hasFill = true;
            } else if (i == 1) {
              vec4 xCol = texture2D(xTexture, cellUv);
              vec3 iconCol = mix(fillColors[i],vec3(1.0), xCol.a);
              stagedColor = iconCol;
              hasFill = true;
            } else if (i == 2) {
              vec4 xCol = texture2D(xTexture, cellUv);
              vec3 iconCol = mix(vec3(1.0), fillColors[i], xCol.a);
              stagedColor = iconCol;
              hasFill = true;
            } else if (i == 3) {
              vec3 fillCurrentColor = col.rgb;
              stagedColor = computeCellFillColor(cellUv, gridColor, fillCurrentColor);
              hasFill = true;
            } else if (i == 4) {
              vec4 xCol = texture2D(xTexture, cellUv);
              vec3 iconCol = mix(gridColor, col.rgb, xCol.a);
              stagedColor = iconCol;
              hasFill = true;
            } else if (i == 5) {
              stagedColor = col.rgb;
          ...
          if (!fillComplete) {
            bool inHoverZone = isInHoverZone(fc, cell);
            float effGapX = max(gapSize, 1.0);
            // Показываем вертикальные линии при ховере или после начала анимации
            // Пропускаем вертикальные линии для белых блоков
            if (m.x < effGapX && !isWhite && (inHoverZone || totalPhaseProgress > 0.39)) { 
              gl_FragColor = vec4(gridColor, 1.0); 
              return; 
            }
            float effGapY = max(gapSize, 1.0);
            // Показываем горизонтальные линии при ховере или в определенном диапазоне анимации
            // Пропускаем горизонтальные линии для белых блоков
            if (!isWhite && (inHoverZone || (totalPhaseProgress >= 0.3 && totalPhaseProgress < (fillPhaseTotalSpan - 0.5)))) {
              if (m.y < effGapY) { 
                vec3 lineColor = inHoverZone ? gridColor.rgb : vec3(1.0, 1.0, 1.0);
                gl_FragColor = vec4(lineColor, 1.0);
                return; 
              }
            }

            // Обработка белых ячеек с учетом заполнения (только для Fill1)
            if (isWhite) {
              // Проверяем, является ли ячейка частью объекта Fill1 (специальный цвет #FEFFFF)
              vec3 srgbCol = pow(clamp(col.rgb, 0.0, 1.0), vec3(1.0 / 2.2));
              vec3 fill1Color = vec3(0.996, 1.0, 1.0); // #FEFFFF в sRGB
              bool isFill1 = distance(srgbCol, fill1Color) < 0.001;
              
              if (isFill1 && hasFill) {
                // Белая ячейка Fill1 с заполнением
                vec3 baseColor = stagedColor;
                vec4 hoveredWhite = applyWhiteHover(vec4(baseColor, col.a), fc, cell, cellTopLeft, blockSize, cellUv, lightColor);
                gl_FragColor = hoveredWhite;
                return;
              }
              // Белая ячейка не Fill1 или без заполнения - обычная обработка
              // Убрали вертикальные линии для белых блоков
            ...
            vec4 outColor = vec4(baseColor, col.a);
            // Enable hover shading while fill animation is running
            if (enableHover) {
              outColor = applyHover(outColor, fc, cell, cellTopLeft, blockSize, cellUv, col.rgb);
            } else {
              outColor = applyIcons(outColor, col, fc, cellIdx, cell);
            }
          ...
          // Белые блоки после завершения заполнения: без вертикальных линий
          if (isWhite) {
            // Убрали вертикальные линии для белых блоков
            vec4 hoveredWhite = applyWhiteHover(col, fc, cell, cellTopLeft, blockSize, cellUv, lightColor);
            gl_FragColor = hoveredWhite; 
    ...
    const easeInOut = (t: number, k = easing) => {
      const clamped = THREE.MathUtils.clamp(t, 0, 1);
      // k=1 → почти линейно; k>1 → мягче на краях
      const x = Math.pow(clamped, k);
      const y = Math.pow(1 - clamped, k);
      return x / (x + y);
    };
    ...
    if (modelUrl) {
      gltfLoader.load(modelUrl, (gltf: any) => {
        ...
        gltf.scene.position.y += objectOffsetY;
        gltf.scene.position.x += objectOffsetX;
    ...
    const tick = () => {
      ...
      if (fillStartTime !== null) {
        ...
        // Ping-pong анимация: от середины к 90% и обратно к середине
        const midPoint = totalRequired / pingPongRatio;
        const maxPoint = totalRequired * pingPongMax; // Максимальная точка - 90%
        const range = Math.max(0, maxPoint - midPoint); // Диапазон от середины до 90%
        const halfCycleDuration = Math.max(1e-4, range * fillPhaseDuration); // Длительность одного направления
        const fullCycleDuration = halfCycleDuration * 2; // Полный цикл = туда + обратно
        const cycleTime = fillElapsed % fullCycleDuration;
        ...
        const totalPhaseProgress = range === 0
          ? midPoint
          : forwardPhase
            ? midPoint + range * eased
            : maxPoint - range * eased;
  ...
  }, [modelUrl, xIconUrl, hIconUrl, blockSize, gapSize, coinSize, xProb, hProb, hoverRadius, trailLength, trailLifetime, maxHoverRotation, easing, pingPongStart, pingPongEnd, enableHover]);
...
const tweakSceneMaterials = (root: THREE.Object3D) => {
  ...
  root.traverse((obj: THREE.Object3D) => {
    ...
    materials.forEach((mat: any) => {
      ...
      if (obj.name === 'Fill1') {
        // Устанавливаем специальный цвет для Fill1 объектов
        replacement = "#" + colors.fill1Color.getHexString();
      }
